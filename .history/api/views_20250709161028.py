...
def game1_view(request):
    ...
    context = {
        'game_id': 1,
        'result_id': result_id,
        'next_game_url': 'game2' # ✅ 将“下一题”的URL传递给模板
    }
...
def game2_view(request):
    ...
    context = {
        'game_id': 2,
        'result_id': result_id,
        'next_game_url': 'game3' # ✅ 将“下一题”的URL传递给模板
    }
...
def game3_view(request):
    ...
    context = {
        ...
        'result_id': result_id,
        'next_game_url': 'game4' # ✅ 将“下一题”的URL传递给模板
    }
...
def game4_view(request):
    ...
    context = {
        ...
        'game_id': 4,
        'result_id': result_id,
        'next_game_url': 'game5' # ✅ 将“下一题”的URL传递给模板
    }
...
def game5_view(request):
    ...
    context = {
        ...
        'game_id': 5,
        'result_id': result_id,
        'next_game_url': 'game6' # ✅ 将“下一题”的URL传递给模板
    }
...
def game6_view(request):
    ...
    context = {
        'game_id': 6,
        'result_id': result_id,
        'next_game_url': 'game7' # ✅ 将“下一题”的URL传递给模板
    }
    return render(request, 'game6.html',context)
def game7_view(request):
    result_id = request.session.get('result_id')
    current_game_name = 'game1'
    current_index = GAME_ORDER.index(current_game_name)
    # 检查是否还有下一关
    if current_index + 1 < len(GAME_ORDER):
        next_game_name = GAME_ORDER[current_index + 1]
        # 使用 reverse() 函数将URL名字安全地转换成实际的URL地址
        next_game_url = reverse(next_game_name)
    else:
        # 如果是最后一关，则没有下一关
        next_game_url = 'result'
    context = {
        'game_name': '日期游戏',
        'game_id': 7,
        'result_id': result_id,
        'next_game_url': next_game_url # ✅ 将“下一题”的URL传递给模板
    }
